...
export function LegislatieTab() {
  ...
  const searchResults = useMemo(() => {
    ...
    return flattenFiles(manifest.root)
        ...
        <Card>
          ...
          <CardContent className="space-y-3 p-4 pt-0 md:p-6 md:pt-0">
            {searchResults.length ? (
              searchResults.map((file) => <FileRow key={file.path} file={file} />)
            ) : (
              <p className="text-sm text-gray-600">Nu am gasit fisiere pentru cautarea curenta.</p>
            )}
          </CardContent>
        </Card>
      ) : (
